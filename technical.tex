...
\ak{programmer interface - To be Defined}
...
\ak{Using  this  technique,}  we  finally solved  the  problem  \ak{of
...
\sid{In particular, the
context is garbage collected only once either the computation is finished or
(in case of a web server) it times out waiting for a client
response.}
%%
\ak{In
particular,  for the  case of  a web  server, the  context is
garbage collected  only once  either when the  computation is
finished,  or when the  computation times  out waiting  for a
